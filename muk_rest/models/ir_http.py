...
class IrHttp(models.AbstractModel):
    ...
    def _rest_authenticate_request(cls, routing, params):
        ...
        def update_request(user):
            ...
            if params.get('with_context', False):
                request.session.context.update(common.parse_value(
                    request.params['with_context'], {}
                ))
            if params.get('with_company', False):
                with_company_id = int(params['with_company'])
                allowed_company_ids = request.session.context.get(
                    'allowed_company_ids', []
                )
