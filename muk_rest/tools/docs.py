...
def generate_routing_docs():
    ...
    def update_components(components, other):
        for section, defs in other.items():
            if section in components:
                components[section].update(defs)
            else:
                components[section] = defs
    ...
    def parse_components(cls):
        ...
        for parent_cls in inspect.getmro(cls):
            update_components(
                components, 
                getattr(parent_cls, '_api_doc_components', {})
            )
        return components
    ...
    for module in modeles_with_controller:
        for _, cls in controllers_per_module[module]:
            ...
            update_components(api_docs_component_values, (parse_components(obj.__class__)))
